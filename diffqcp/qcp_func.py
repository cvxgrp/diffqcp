"""DEPRECATED.
Keeping for now for backward compatibility, specifically for some tests. Will remove in next version.
(Or when I feel like cleaning things up).
"""
...
def compute_derivative(P: torch.Tensor | spmatrix,
    ...
    solution : tuple[torch.Tensor | np.ndarray | list[float],
        ...
        This solution can be passed in as a tuple of torch.Tensors or np.ndarrays.
        Provide the solution vectors according to (x^star, y^star, s^star).
