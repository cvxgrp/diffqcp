...
def test_prob_data_upper_P_scipy(device):
    ...
    new_A = utils.get_random_like(A, randomness = lambda n: np.random.normal(0, 1e-6, size=n))
    new_P_upper = utils.get_random_like(P_upper, randomness = lambda n: np.random.normal(0, 1e-6, size=n))

    data.A = new_A
    data.P = new_P_upper

    full_P = utils.form_full_symmetric(new_P_upper.todense())
    ...
    # these index tests are not really checking anything `ProblemData` wise.
    indices = new_P_upper.nonzero()
    P_rows, P_cols = indices[0], indices[1]
    assert torch.allclose(diffqcp_utils.to_tensor(P_rows, dtype=torch.int64, device=device),
                          data.P_rows)
    assert torch.allclose(diffqcp_utils.to_tensor(P_cols, dtype=torch.int64, device=device),
                          data.P_cols)
    assert torch.allclose(diffqcp_utils.to_tensor(new_P_upper.indptr, dtype=torch.int64, device=device),
                          data.Pcrow_indices)
    assert torch.allclose(diffqcp_utils.to_tensor(new_P_upper.indices, dtype=torch.int64, device=device),
                          data.Pcol_indices)
